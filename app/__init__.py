...
def create_app():
    ...
    def attach_request_id_and_log(resp: Response):
        ...
        if dt_ms is not None and dt_ms >= SLOW_REQUEST_MS:
            _log_event("slow_request", level="warning", endpoint=request.endpoint, status=resp.status_code, duration_ms=dt_ms)
        if resp.status_code >= 400 and not request.path.startswith("/static/"):
            _log_event("http_error", level="warning", endpoint=request.endpoint, status=resp.status_code, duration_ms=dt_ms)
        return resp
    ...
    try:
        ...
        def handle_rate_limit(e):
            _log_event("rate_limited", level="warning", endpoint=request.endpoint, status=429, limit=str(getattr(e, "description", ""))[:200])
